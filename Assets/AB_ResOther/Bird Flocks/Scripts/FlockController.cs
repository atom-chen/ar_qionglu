...
public class FlockController : WriteItem
{
    ...
    public float _minSpeed = 3.0f;				// minimum random speed
    public float _maxSpeed = 5.0f;				// maximum random speed
    ...
    {    	_thisT = transform;
    	...
    	if(_randomPositionTimer > 0) InvokeRepeating("SetFlockRandomPosition", _randomPositionTimer, _randomPositionTimer); // > C
              goodsEnum = GoodsWriteEnum.None;
      goodsPositionEnum = PlaneMode.GROUND;
    ...
    public void AddChild(int amount){
	    ...
	    {
	        int random = Random.Range(0, _childPrefab.Length);
    		FlockChild obj = (FlockChild)Instantiate(_childPrefab[random]);	
    	    obj._spawner = this;
    	    _roamers.Add(obj);
