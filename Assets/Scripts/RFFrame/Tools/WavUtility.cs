...
public class WavUtility
{
    ...
    {
        if (!filePath.Contains(Application.persistentDataPath) && !filePath.Contains(Application.dataPath))
        {
    ...
    #endregion wav file bytes to Unity AudioClip conversion methods
    ...
    public static byte[] FromAudioClip(AudioClip audioClip, out string filepath, bool saveAsFile = true, string dirname = "recordings")
    {
        ...
        if (saveAsFile)
        {
            filepath = string.Format("{0}/{1}/{2}.{3}", Application.persistentDataPath, dirname, "XCY", "wav");
            Debug.Log(filepath);
    ...
    #endregion write .wav file functions
